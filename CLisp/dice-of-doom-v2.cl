...
(defmacro play (opponent)
  (let ((fun (intern (concatenate 'string
								  "PLAY-VS-"
								  (string-upcase (string opponent))))))
	`(funcall #',fun (game-tree (gen-board) 0 0 t))))
...
(defun handle-human (tree)
  ...
  (let ((moves (caddr tree))
	...
	(labels ((print-moves (moves n)
	  ...
	  (print-moves moves 1))
	(fresh-line)
	(cadr (lazy-nth (1- (read-move nmove)) moves))))

(defun read-move (n)
  (let ((move (read)))
	(if (not (and (numberp move) (<= move n)))
	  (read-move n)
	  move)))
