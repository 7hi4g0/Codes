        ...
        if let Some(Button::Keyboard(key)) = e.press_args() {
            if let Some(ind) = self.selected_cell {
                ...
                match key {
                    ...
                    Key::Up => self.selected_cell = Some([ind[0], (ind[1] + 8) % 9]),
                    Key::Down => self.selected_cell = Some([ind[0], (ind[1] + 1) % 9]),
                    Key::Left => self.selected_cell = Some([(ind[0] + 8) % 9, ind[1]]),
                    Key::Right => self.selected_cell = Some([(ind[0] + 1) % 9, ind[1]]),
                    _ => {}
                }
            }
        }
