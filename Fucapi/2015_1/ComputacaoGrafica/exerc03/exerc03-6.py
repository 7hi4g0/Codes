...
def draw():
    global previous_time, dx, dy, dtheta, posx, posy, scale, scalex, scaley, theta
    ...
    glScale(0.1 * scale, 0.1 * scale, 1)
    desenhaGrade(-10, 10, -10, 10, unit=1)

    # --- INICIO DO DESENHO DA TELA ---
    posx = posx + delta_time * dx
    posy = posy + delta_time * dy
    theta = (theta + delta_time * dtheta * 36) % 360

    color_time = current_time % 6000
    color_proportion = (color_time % 2000) / 2000.0
    if color_time < 2000:
        glColor(1 - color_proportion, color_proportion, 0)
    elif color_time < 4000:
        glColor(0, 1 - color_proportion, color_proportion)
    else:
        glColor(color_proportion, 0, 1 - color_proportion)
...
def keyboard(char, x, y):
    global dx, dy, dtheta, posx, posy, scale, scalex, scaley, theta
...
if __name__ == '__main__':
    ...
    glutCreateWindow("2a Avaliacao Parcial - 6")
