...
//#define	ponderado	//Descomente para usar somatório ponderado
#define	notaProcurar	4.5
#define	notaAlterar	9.5
#define	pesoAlterar	3
#define	notaExcluir	2.0
...
int main(void) {
	...
	printf("Exclui nota %.2f: %s\n", notaExcluir, excluir(&lista, notaExcluir) ? "Sim" : "Não");
	imprimeLista(lista);
...
int excluir(avaliacao **lista, float nota) {
	avaliacao **percorre;
	
	percorre = lista;
	
	while (*percorre != NULL) {
		if ((*percorre)->nota == nota) {
			avaliacao *apagar;
